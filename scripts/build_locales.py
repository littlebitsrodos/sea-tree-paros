...
def build_faqpage_jsonld(loc_translations):
    """Build a FAQPage JSON-LD block from the faq.* translations.
    Returns the full <script> element as a string, or '' if no FAQ keys."""
    faq = loc_translations.get("faq")
    if not faq:
        return ""
    main_entity = []
    i = 1
    while True:
        q_obj = faq.get(f"q{i}")
        if not q_obj or "q" not in q_obj or "a" not in q_obj:
            break
        main_entity.append({
            "@type": "Question",
            "name": q_obj["q"],
    ...
    body = body.replace("\n", "\n  ")
    return (
        '\n  <!-- FAQPage (generated by build_locales.py) -->\n'
...
def inject_faqpage(html, loc_translations):
    block = build_faqpage_jsonld(loc_translations)
    if not block:
        return html
    # Strip any previously-injected FAQPage block (idempotent rebuild).
    html = re.sub(
    ...
    # Insert after the last existing JSON-LD block in the head.
    match = None
    for m in re.finditer(r'</script>\s*(?=\n\s*(?:<!--|<link|<meta|<script|</head))', html):
        match = m
    if match is None:
        return html + block
    idx = match.end()
